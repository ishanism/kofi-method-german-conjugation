...
def get_sentence_template(tense, form, infinitive, person=""):
    ...
    sentence_template = templates.get(tense, "").format(
        form, infinitive, person_with_space
    )
    if tense == "Infinitive":
        sentence = sentence_template.replace("[...]", f"{{c1::{tense}}}")
    else:
        sentence = sentence_template.replace("[...]", f"{{c1::{form}}}")
    return sentence
...
def create_anki_deck(verb_data):
    ...
    with open("results.txt", "w+") as file:
        ...
        for verb_name, note, conjugations in verb_data:
            ...
            for tense, conjs in conjugations.items():
                ...
                if tense in ["Partizip I", "Partizip II"]:
                    ...
                    for form in forms:
                        ...
                        sentence = get_sentence_template(tense, form, verb_name)
                        file.write(f"{sentence}\n")
                        tags = get_tags(tense, verb_name)
                        audio = conjugated_audio(verb_name, form)
                        audio_files.append(audio)
                        all_notes.append(
                            make_note(kofi_model, sentence, note, tags, audio)
                        )
                else:
                    ...
                    for i in range(0, len(forms), 2):
                        ...
                        if tense == "Imperativs":
                            ...
                            for i, imperativ in enumerate([singular, plural]):
                                sentence = get_sentence_template(
                                    tense, imperativ, verb_name
                                )
                        ...
                        # PRASENS, Konjunktiv II: Präteritum,
                        # Indikativ: Präteritum
                        sentence = get_sentence_template(tense, form, verb_name, person)
                        file.write(f"{sentence}\n")
            ...
            sentence = get_sentence_template(
                "Infinitiv", verb_name, third_person_present
            )
            file.write(f"{sentence}\n")
